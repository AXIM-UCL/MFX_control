...
ylims=[300, 510, 1000, 1650, 1950, 2300]
...
for i in range(n_sphs):
    ...
    result_pad=np.pad(result, (pad_h, pad_w), constant_values=np.mean(result[:5, :5]), mode='constant')
    result2=cv2.matchTemplate(result_pad, -np.log(ball_window), method=cv2.TM_CCORR_NORMED)
    plt.imshow(result)

    #result2=cv2.GaussianBlur(result2,(7,7),0)
    indmax=np.argmax(result2)
    ymax, xmax=np.unravel_index(indmax, result2.shape)
    ymax=ymax+ylims[i]
    xmax=xmax
    ...
    _,th = cv2.threshold(window,0.6,0, cv2.THRESH_TOZERO)
...
for angle in angles:
    ...
    for i in range(n_sphs):
        ...
        result_pad=np.pad(result, (pad_h, pad_w), constant_values=np.mean(result[:5, :5]), mode='constant')
        result2=cv2.matchTemplate(result_pad, -np.log(ball_window), method=cv2.TM_CCORR_NORMED)
        result2=np.nan_to_num(result2, 0)
        indmax=np.argmax(result2)
        ymax, xmax=np.unravel_index(indmax, result2.shape)
        ymax=ymax+ylims[i]
        xmax=xmax
        window=ffc_blur[ymax-win_size:ymax+win_size, xmax-win_size:xmax+win_size]
        window=-np.log(window).astype(np.float32)
        window=np.nan_to_num(window, nan=np.median(window), posinf=np.median(window), neginf=np.median(window))
        sub_xv, sub_yv = (xv[ymax-win_size:ymax+win_size, xmax-win_size:xmax+win_size], yv[ymax-win_size:ymax+win_size, xmax-win_size:xmax+win_size])
        #window=(255*window/np.amax(window)).astype(np.uint8)
        _,th = cv2.threshold(window,0.6,0, cv2.THRESH_TOZERO)
        x_cen=np.sum(sub_xv*th)/np.sum(th)
        y_cen=np.sum(sub_yv*th)/np.sum(th)
        if(np.isfinite(x_cen) and np.isfinite(y_cen)):
            yloc.append(y_cen)
            xloc.append(x_cen)
        else:
            if(len(xloc)>0):
...
xmed=x_pos[:,2]; ymed=y_pos[:,2]
xbot=x_pos[:,4]; ybot=y_pos[:,4]
