# -*- coding: utf-8 -*-
"""
Created on Wed May 19 09:33:20 2021
...
detector_rows = 402  # Vertical size of detector [pixels].
...
eta=np.radians(0.086) #In plane rotation(skew)
...
source=r'C:\Data\21_10_25\CT_900proj_abs_calib_phantom\\'
input_dir = source+'FFC\\'
output_dir = source+'CBReconstructions251_angle+_213v\\'
...
for i in range(num_of_projections):
    ...
    im = imread(join(input_dir, file_name))
    T = np.float32([[1, 0, -x_disp[i]], [0, 1, -y_disp[i]]])
    proj=-np.log(im)
    proj = cv2.warpAffine(proj, T, (512, 402))
    projections[:, i, :] = proj
