...
path=r'D:\Data\22_01_27\CT_100proj_abs_calib_phantom\FFC\\'
...
file_name=path+'ffc_00025.tif.tif'
ffc=tifffile.imread(file_name)
ffc_blur=cv2.GaussianBlur(ffc,(5,5),0)
sz=43;sty=911;stx=1178 #angle10_2 aft crash
...
N_proj=90
angle=25
file_name=path+'ffc_000'+str(angle)+'.tif.tif'
ffc=tifffile.imread(file_name)
ffc_blur=cv2.GaussianBlur(ffc,(5,5),0)
...
ylims=[500, 740, 860, 1000, 1150, 1290, 1400, 1515, 1750]
yloc=[];xloc=[];rs=[]
...
us_top_dist, vs_top_dist, angle1=tools_geometry.get_points_dists(xtop, ytop, N_proj, type=0)

us_med_dist, vs_med_dist, an=tools_geometry.get_points_dists(xmed, ymed, N_proj, type=1)

us_bot_dist, vs_bot_dist, angle2=tools_geometry.get_points_dists(xbot, ybot, N_proj, type=1)

us_dist_stack=np.stack((us_top_dist, us_med_dist, us_bot_dist))
vs_dist_stack=np.stack((vs_top_dist, vs_med_dist, vs_bot_dist))


param_dist=tools_geometry.calculate_parameters_Yang(us_dist_stack, vs_dist_stack, angle1, angle2, l=3.72)
